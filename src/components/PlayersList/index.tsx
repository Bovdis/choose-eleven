        ...
        <tr>
          <td colSpan={6}>
            <ul className={styles.playerListTablePagination}>
              <FontAwesomeIcon icon={faArrowLeft} onClick={() => handlePageChange(currentPage - 1)} />
              {renderPageNumbers && renderPageNumbers.map(number => {
                return (
                  <li
                    className={(currentPage === number ? styles.active : styles.inactive)}
                    key={number}
                    id={number.toString()}
              ...
              <FontAwesomeIcon icon={faArrowRight}  onClick={() => handlePageChange(currentPage + 1)} />
            </ul>
          </td>
        </tr>
      </tfoot>
